...
### Проблемы:
<ul>
  <li>Нестабильность переключения полноэкранного режима.</li>
  <li>В каждом модуле приходилось вручную дублировать код для отображения победной сцены.</li>
  <li>Зависание мини-игры "Memo" при неверной паре.</li>
  <li>Некоторые изображения выходили за границы экрана при различных разрешениях.</li>
</ul>

### Решения:
<ul>
  <li>Была реализована вспомогательная структура ScreenManager, позволяющая централизованно управлять состоянием экрана. Также при каждом переключении пересчитывались размеры карточек и позиции элементов.</li>
  <li>Функция show_victory_sequence() была выделена как общая и подключалась во всех мини-играх для переиспользования.</li>
  <li>Была введена переменная fail_time_start, которая фиксирует момент ошибки и запрещает следующий ввод до завершения задержки в 1 секунду.</li>
  <li>Все изображения стали масштабироваться динамически в зависимости от размеров окна (get_screen_size()), и был внедрён расчёт card_size, адаптирующий размеры карточек к текущему экрану.</li>
</ul>
