...
pygame.display.set_caption("Выживание на острове")
FONT = pygame.font.SysFont("arial", 28)  # Создание шрифта для текста

def main():
    inventory = Inventory()

    show_message(SCREEN, "Вы очнулись на острове...", "Нажмите любую клавишу")

    if not maze_game(SCREEN, inventory):
        return

    if not match3_game(SCREEN, inventory):
        return

    if not hangman_game(SCREEN, inventory):
        return

    if not tictactoe_game(SCREEN, inventory):
        return

    if not memo_game(SCREEN, inventory):
        return

    show_message(SCREEN, "Вы собрали еду и уплыли с острова!", "Победа!", color=(0, 180, 0))
    pygame.quit()
    sys.exit()
